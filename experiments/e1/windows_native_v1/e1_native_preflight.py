...
def _qualify_windows_runtime(
    *,
    ...
    # Full host pip-inventory digest must match the frozen value.
    ...
    pip_raw = json.loads(pip_result.stdout)
    pip_inventory: list[dict[str, str]] = []
    for item in pip_raw:
        pip_inventory.append({"name": str(item["name"]), "version": str(item["version"])})
    pip_inventory.sort(key=lambda item: item["name"])
    host_inventory_digest = canonical_sha256(pip_inventory)
    if host_inventory_digest != _EXPECTED_ENVIRONMENT["host_inventory_digest"]:
        raise RuntimeError(
            f"host inventory digest mismatch: expected "
            f"{_EXPECTED_ENVIRONMENT['host_inventory_digest']}, "
            f"observed {host_inventory_digest}"
        )
    ...
    # Verify model safetensors digest on Windows. Fail-closed: an unresolved
    # digest (snapshot/model file not found) is a hard failure on the Windows
    # target, not a silent skip.
    model_digest = _compute_model_safetensors_digest()
    if model_digest is None:
        raise RuntimeError("model safetensors digest could not be computed")
    if model_digest != MODEL_CONTENT_DIGEST:
        raise RuntimeError(
            f"model safetensors digest mismatch: expected {MODEL_CONTENT_DIGEST}, "
            f"observed {model_digest}"
        )

    # Verify the complete inherited runtime identity — not just the subset
    # above, but also OS build, architecture, Python implementation, Python
    # executable SHA-256, transformers/accelerate versions, and NVIDIA driver.
    import importlib.metadata

    for pkg_name, env_key in (
        ("transformers", "transformers_version"),
        ("accelerate", "accelerate_version"),
    ):
        try:
            observed_ver = importlib.metadata.version(pkg_name)
        except importlib.metadata.PackageNotFoundError:
            observed_ver = "<not installed>"
        if observed_ver != _EXPECTED_ENVIRONMENT[env_key]:
            mismatches[env_key] = {
                "expected": _EXPECTED_ENVIRONMENT[env_key],
                "observed": observed_ver,
            }

    # OS build and architecture.
    if platform.machine() != _EXPECTED_ENVIRONMENT["architecture"]:
        mismatches["architecture"] = {
            "expected": _EXPECTED_ENVIRONMENT["architecture"],
    ...
    if str(build) != _EXPECTED_ENVIRONMENT["os_build"]:
        mismatches["os_build"] = {
            "expected": _EXPECTED_ENVIRONMENT["os_build"],
            "observed": str(build),
        }

    # Python implementation and executable SHA-256.
    if platform.python_implementation() != _EXPECTED_ENVIRONMENT["python_implementation"]:
        mismatches["python_implementation"] = {
            "expected": _EXPECTED_ENVIRONMENT["python_implementation"],
            "observed": platform.python_implementation(),
        }
    import hashlib as _hashlib
    import sys as _sys

    exe_digest = _hashlib.sha256(Path(_sys.executable).read_bytes()).hexdigest()
    if exe_digest != _EXPECTED_ENVIRONMENT["python_executable_sha256"]:
        mismatches["python_executable_sha256"] = {
            "expected": _EXPECTED_ENVIRONMENT["python_executable_sha256"],
            "observed": exe_digest,
        }
    ...
    driver_ver = nvidia_result.stdout.strip()
    if driver_ver != _EXPECTED_ENVIRONMENT["nvidia_driver_version"]:
        mismatches["nvidia_driver_version"] = {
            "expected": _EXPECTED_ENVIRONMENT["nvidia_driver_version"],
            "observed": driver_ver,
        }
    ...
    return {
        ...
        "python_executable_sha256": exe_digest,
        ...
        "host_package_count": len(pip_inventory),
        "tokenizer_asset_digest": asset_digest,
        "model_safetensors_digest": model_digest,
    }
...
def _find_hf_snapshot() -> Path | None:
    ...
    ``HF_HOME`` points ABOVE the ``hub/`` directory (the HuggingFace
    convention is ``$HF_HOME/hub/models--...``), so its candidate hub-cache
    directory is ``Path(env_home) / "hub"``. ``HF_HUB_CACHE`` (and the legacy
    ``HUGGINGFACE_HUB_CACHE``) points DIRECTLY at the hub cache, so it is used
    as-is. The two env vars are resolved separately so a snapshot found under
    one layout is not masked by the other.
    """

    import os

    # Each entry is a hub-cache directory (the parent of ``models--...``).
    hub_cache_dirs: list[Path] = []
    env_home = os.environ.get("HF_HOME")
    if env_home:
        # HF_HOME is the parent of ``hub/``; the cache lives under ``hub/``.
        hub_cache_dirs.append(Path(env_home) / "hub")
        # Fall back to treating HF_HOME itself as a hub cache for caches laid
        # out directly under it (older/non-standard layouts).
        hub_cache_dirs.append(Path(env_home))
    env_cache = os.environ.get("HF_HUB_CACHE") or os.environ.get("HUGGINGFACE_HUB_CACHE")
    if env_cache:
        hub_cache_dirs.append(Path(env_cache))
    # Standard default hub-cache locations.
    hub_cache_dirs.append(Path.home() / ".cache" / "huggingface" / "hub")
    hub_cache_dirs.append(Path("C:/huggingface_cache/hub"))
    hub_cache_dirs.append(Path("artifacts") / "e0h-windows-native-v2" / "hf-cache")

    for cache_dir in hub_cache_dirs:
