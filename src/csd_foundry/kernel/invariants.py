...
def _validate_reassess(
    ...
    if not new_evidence_ids <= set(after.evidence_by_id()):
        violations.append(Violation("INV-18", "reassessment omitted issued evidence"))
    if not new_basis_ids <= set(after.bases_by_id()):
        violations.append(Violation("G-INV-11", "reassessment omitted established basis"))

    resulting_source = event.source_state or before.source_state
    resulting_assurance = event.assurance or before.assurance
    bases = after.bases_by_id()
    expected_source = frozenset(
        basis_id
        for basis_id in before.current_source_basis_ids | new_basis_ids
        if (basis := bases.get(basis_id)) is not None
        and basis.kind is BasisKind.SOURCE
        and basis.claim == resulting_source.value
        and _basis_is_supported(basis, after.evidence_by_id())
    )
    expected_verdict = frozenset(
        basis_id
        for basis_id in before.current_verdict_basis_ids | new_basis_ids
        if (basis := bases.get(basis_id)) is not None
...
def _validate_retirement(after: ControlState) -> list[Violation]:
    violations: list[Violation] = []
...
def validate_event_transition(
    ...
    if isinstance(event, RetireControl):
        return tuple(_validate_retirement(after))
