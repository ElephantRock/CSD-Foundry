"""Scenario-wide symbolic identities and leakage-safe E1 split manifests."""
...
def _add_event_ids(
    ...
    *,
    ...
    if isinstance(event, RetireControl):
        evidence_item = event.retirement_evidence
        evidence.add(evidence_item.evidence_id)
        dependencies.update(evidence_item.dependencies)
        if evidence_item.profile_id is not None:
            profiles.add(evidence_item.profile_id)
    ...
    if isinstance(event, RequestReassessment):
        requests.add(event.request_id)
        return
    if isinstance(event, (AdvanceClock, RecordHeartbeat)):
        return
...
def _collect_scenario_ids(spec: ScenarioSpec) -> _NamespaceIds:
...
def _render_case(case: ScenarioCase, ids: _IdMap) -> dict[str, object]:
    if isinstance(case, TransitionCase):
        return {
...
def _case_coordinates(spec: ScenarioSpec) -> tuple[dict[str, object], ...]:
    ...
    for position, case in enumerate(spec.cases):
        ...
        for depth, raw_part in enumerate(structural_parts):
            while len(label_maps) <= depth:
                label_maps.append({})
            match = _COORDINATE_TOKEN.fullmatch(raw_part)
            if match is None:
                raise FamilySplitError(f"invalid case coordinate token: {raw_part}")
            ordinal_text, token = match.groups()
            labels = label_maps[depth]
            if token not in labels:
                labels[token] = f"C{len(labels)}"
            rendered_parts.append(
                {
                    "ordinal": None if ordinal_text is None else int(ordinal_text),
                    "equivalence": labels[token],
                }
            )
        coordinates.append(
            {
        ...
        "cases": [
            {
                ...
                "case": _render_case(case, ids),
            }
            for case, coordinate in zip(spec.cases, coordinates, strict=True)
        ],
    }


def _family_material(spec: ScenarioSpec) -> dict[str, object]:
    namespaces = _collect_scenario_ids(spec)
    coordinates = _case_coordinates(spec)
    best_shape: dict[str, object] | None = None
    best_bytes: bytes | None = None
    for ids in _candidate_maps(namespaces):
        candidate = _render_scenario(spec, ids, coordinates)
...
def derive_scenario_family_identity(spec: ScenarioSpec) -> ScenarioFamilyIdentity:
    """Derive one scenario-wide identity over executable structure and coordinates."""
