...
class AssumptionPolicySigningPayload:
    ...
    def build(
        ...
        *,
        ...
        if approval_policy.authority_root_digest != policy.authority_root_digest:
            raise AssumptionPolicyActivationContractError(
                "ASSUMPTION_POLICY_SIGNING_PAYLOAD_APPROVAL_ROOT_MISMATCH"
            )
        if signature_profile.key_authority_root_digest != policy.authority_root_digest:
            raise AssumptionPolicyActivationContractError(
                "ASSUMPTION_POLICY_SIGNING_PAYLOAD_SIGNATURE_ROOT_MISMATCH"
            )
        unsigned = {
...
class AssumptionPolicyLedgerEntryV3:
    ...
    def __post_init__(self) -> None:
        ...
        bindings = (
            ...
            # authority-root defense in depth: payload <-> approval/profile roots
            (
                payload.authority_root_digest,
                self.approval_policy.authority_root_digest,
            ),
            (
                payload.authority_root_digest,
                self.signature_profile.key_authority_root_digest,
            ),
        )
...
class AssumptionPolicyLedgerV3:
    ...
    def __post_init__(self) -> None:
        # Runtime version enforcement: every entry must be exactly V3.
...
# --- exact signature-target validation (correction 1) ---------------------


def require_policy_signature_target(
    *,
    signed_digest: str,
    signing_payload_digest: str,
) -> None:
    """Require exact, case-sensitive equality between the signed digest and the
    signing-payload digest.

    Both formats are validated first; any different target raises
    ``ASSUMPTION_POLICY_SIGNATURE_TARGET_MISMATCH``.
    """

    require_digest(signed_digest, "ASSUMPTION_POLICY_SIGNATURE_TARGET_MISMATCH")
    require_digest(signing_payload_digest, "ASSUMPTION_POLICY_SIGNATURE_TARGET_MISMATCH")
    if signed_digest != signing_payload_digest:
        raise AssumptionPolicyActivationContractError("ASSUMPTION_POLICY_SIGNATURE_TARGET_MISMATCH")


# --- message-binding conformance vector (correction 2) --------------------


def deterministic_policy_signature_bytes(
    *,
    algorithm: str,
    ...
    """Deterministic conformance vector committing to all four decision-critical
    inputs including ``signed_digest``.

    Changing only ``signed_digest`` changes the generated bytes. This is
    explicitly a non-production conformance helper; it makes no cryptographic
    claim and must never be used to authorize production material. It may be
    reused by the executable preparer (PR #50) after the envelope merges.
    """

    require_token(algorithm, "ASSUMPTION_POLICY_ALGORITHM_INVALID")
    require_token(
        verification_profile,
        "ASSUMPTION_POLICY_VERIFICATION_PROFILE_INVALID",
    )
    if type(public_key_bytes) is not bytes or not public_key_bytes:
        raise AssumptionPolicyActivationContractError("ASSUMPTION_POLICY_PUBLIC_KEY_BYTES_INVALID")
    require_digest(signed_digest, "ASSUMPTION_POLICY_SIGNED_DIGEST_INVALID")
    payload = (
    ...
    return hashlib.sha256(b"ASSUMPTION_POLICY_CONFORMANCE_SIGNATURE\0" + payload).digest()


# --- executable commit-version gate (correction 3) ------------------------


def require_activatable_policy_commit(
    value: object,
) -> AssumptionAuthorityPolicyCommitV3:
    """Accept only the exact V3 commit type; reject V2 or foreign objects.

    Commit/2 remains independently parseable (its class and parser are
    untouched) but is not activatable by the executable preparer.
    """

    if type(value) is AssumptionAuthorityPolicyCommitV3:
        return value
    raise AssumptionPolicyActivationContractError(
        "ASSUMPTION_POLICY_COMMIT_VERSION_NOT_ACTIVATABLE"
    )
...
# --- complete predecessor validation (correction 6) -----------------------


def validate_successor_position_v3(
    head: AssumptionPolicyLedgerEntryV3,
    candidate: AssumptionPolicyLedgerEntryV3,
) -> None:
    """Validate that ``candidate`` is a proper successor of ``head``.

    Requires the complete predecessor pair (policy digest + commit receipt)
    to match before enforcing monotonic effective-sequence ordering.
    """

    payload = candidate.signing_payload
    predecessor_matches = (
        payload.predecessor_policy_digest == head.policy.policy_digest
        and payload.predecessor_commit_receipt_digest == head.policy_commit.commit_receipt_digest
    )
    if not predecessor_matches:
        raise AssumptionPolicyActivationContractError("ASSUMPTION_POLICY_CHAIN_V3_FORK")
    if payload.effective_from_sequence <= head.signing_payload.effective_from_sequence:
        raise AssumptionPolicyActivationContractError(
...
def order_policy_entries_v3(
    ...
    while True:
        ...
        validate_successor_position_v3(current, successor)
        current = successor
