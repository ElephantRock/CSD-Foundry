...
def validate_policy_overlap(policy: AssumptionAuthorityPolicy) -> None:
    ...
    for index, exception_left in enumerate(policy.duty_exceptions):
        for exception_right in policy.duty_exceptions[index + 1 :]:
            if duty_exceptions_overlap(exception_left, exception_right):
                pair = ",".join(
                    sorted(
                        (
                            exception_left.exception_id,
                            exception_right.exception_id,
                        )
                    )
                )
                raise AssumptionPolicyActivationContractError(
...
def duty_exceptions_overlap(
    ...
    if not _scopes_intersect(left.scope_ids, right.scope_ids):
        return False
    if not set(left.assumption_materialities).intersection(right.assumption_materialities):
        return False
