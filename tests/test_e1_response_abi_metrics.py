...
def test_git_history_source_commit_gate_binds_real_implementation_commit():
    """Slice-aware provenance gate for the A0b2 response ABI receipt.

    When HEAD changes exactly the six v4 artifacts (A0b2 artifact commit
    context), enforce S→A adjacency and receipt source_commit exactly.

    In a merged/successor context, verify the frozen artifacts and receipt
    remain byte-consistent without requiring HEAD to be the A0b2 artifact commit.
    """
    ...
    receipt_text = _git("show", "HEAD:data/e1/v4/a0b2_receipt.json")
    receipt = json.loads(receipt_text)
    committed_source_commit = receipt["source_commit"]

    # Check if HEAD changes exactly the A0b2 v4 artifacts.
    ...
    v4_artifact_set = {f"data/e1/v4/{name}" for name in expected_artifacts}

    if head_diff == v4_artifact_set:
        # Direct A0b2 artifact commit context: enforce S→A adjacency exactly.
        ...
        # byte-consistent through Git blob identity.
        ...
        for name in expected_artifacts:
            rel = f"data/e1/v4/{name}"
            frozen_blob = _git("rev-parse", f"{frozen_commit}:{rel}")
            current_blob = _git("hash-object", rel)
            assert current_blob == frozen_blob, f"frozen A0b2 artifact changed: {rel}"
