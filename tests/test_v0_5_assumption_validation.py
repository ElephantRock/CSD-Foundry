...
def test_committed_assumption_vectors_validate_independently() -> None:
    ...
    assert report.accepted_vector_count == 13
    assert report.rejected_vector_count == 12
    assert len(report.accepted_registry_roots) == 13
    assert len(report.accepted_decision_digests) == 13
    assert len(report.rejected_failure_codes) == 12
    assert report.vector_catalog_digest == (
        "sha256:4e3e9d997b8705bebfabb0d73ec91d21fd2577c5d85d6e20e5ee1e2f275eb776"
    )
