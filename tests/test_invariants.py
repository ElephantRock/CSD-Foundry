...
from fixtures.v0_1.scenarios import m01, m09
...
def test_reactivation_is_rejected() -> None:
    ...
    evidence = tuple(
        replace(item, status=EvidenceStatus.CURRENT)
        if item.evidence_id == "EV-N17-001"
        else item
        for item in after.evidence
    )
    invalid = replace(after, evidence=evidence)
    assert "INV-18" in {
        violation.invariant_id for violation in validate_transition(after, invalid)
    }
