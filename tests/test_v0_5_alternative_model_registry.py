...
def test_all_resolved_challenges_restore_admitted() -> None:
    store = InMemoryRegistryStore()
    registry = AlternativeModelRegistry(store)
    _, admitted = _propose_and_admit(store)
    challenged = registry.apply(_challenge(admitted, "challenge:alpha", 3))
    ...
    # With every challenge resolved, the model is restored to ADMITTED and the
    # overlay collapses.
    assert restored.separation_status == STANDING_ADMITTED
    assert restored.standing == STANDING_ADMITTED
